...
def send_message(token, chat_id, text):
    url = f'https://api.telegram.org/bot{token}/sendMessage'  # URL API Telegram per invio messaggi
    payload = {
        'chat_id': chat_id,  # ID della chat dove inviare il messaggio
        'text': text,  # Contenuto del messaggio
        'parse_mode': 'HTML'  # Abilita la formattazione HTML nel messaggio
    }
    response = requests.post(url, data=payload)  # Invia POST request
    return response.json()  # Ritorna la risposta in formato JSON
    ...
    try:
        ...
        if(type == "allag"):
            ...
            if row:
                ...
                if event:
                    MESSAGE = f"🚨 <b>ALLERTA ALLAGAMENTO</b>\n📅 Data: <i>{date}</i>\n🕒 Ora: <i>{hour}</i>"
                    message[0] = MESSAGE
        ...
        if(type == "batt"):
            ...
            if results:
                ...
                if(event):
                    MESSAGE = f"🔋 <b>BATTERIA SCARICA</b>\n📦 Case interessate:{sG}{sP}\n⚠️ Si consiglia la sostituzione."
                    message[1] = MESSAGE
                    events[1] = True
            else:
                print("Nessun dato trovato")
        ...
        if(type == "vibr"):
            ...
            if row:
                ...
                if event:
                    MESSAGE = f"🚨 <b>ALLERTA VIBRAZIONE RILEVATA</b>\n📅 Data: <i>{date}</i>\n🕒 Ora: <i>{hour}</i>"
                    message[2] = MESSAGE
                    events[2] = True
            else:
                print("Nessun dato trovato")
